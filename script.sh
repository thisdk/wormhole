...
docker run --restart=always --network bridge --name nginx -p 443:443 -p 443:443/udp -v /etc/nginx/nginx.conf:/etc/nginx/nginx.conf -d nginx

docker run --restart=always --network bridge --name sing-box -v /etc/sing-box:/etc/sing-box -d ghcr.io/sagernet/sing-box run -c /etc/sing-box/config.json
